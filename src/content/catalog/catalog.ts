...
function calculateInvariantReport(): CatalogueInvariantReport {
  ...
  const minigameIds = minigames.map(({ id }) => id);
  const creatureIds = creatures.map(({ id }) => id);
  ...
  return {
    ...
    uniqueIds: unique([...buildingIds, ...minigameIds, ...creatureIds]) && unique(cities.map(({ id }) => id)),
    crossReferences,
    restStopsComplete,
  };
}
...
export function assertCatalogueInvariants(): void {
  ...
  if (!report.uniqueIds || !report.crossReferences || !report.restStopsComplete) throw new Error("Catalogue relationship invariant failed.");
}
